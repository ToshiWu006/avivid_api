...
class getCouponRelatedApiController extends Controller
{
    ...
    public function get_coupon_status(Request $request)
    {
        ...
        $return_results = array(
            "status" => $coupon_status, "id" => $coupon_id, "customer_type" => $coupon_customer_type,
    ...
    public function get_all_coupon_status(Request $request)
    {
        ...
        if ($coupon_status) {
            ...
            $return_results = array(
                "avg_budget" => 0, "remain_time" => 0, "remain_budget" => 0, "coupon_limit" => 'limit-bill=0',
    ...
    public function get_coupon_model(Request $request)
    {
        ...
        $coupon_model_result = array(
            "model_key_js" => $model_key_js, "model_value" => $model_value, "model_intercept" => $model_intercept,
    ...
    public function get_coupon(Request $request)
    {
        ...
        $coupon = DB::connection('rhea1-db0')->table('addfan_activity')
            ...
            ->select(
                'addfan_activity.title',
                'addfan_activity.coupon_description',
                'addfan_coupon.coupon_code',
                'addfan_activity.link_code',
                'addfan_activity.coupon_type',
                'addfan_activity.coupon_amount',
                'addfan_activity.coupon_code_mode',
                'addfan_activity.coupon_time_limit',
                'addfan_activity.coupon_limit',
                'addfan_activity.coupon_url',
                'addfan_activity.coupon_waitingTime',
                'addfan_activity.promotion_switch',
                'addfan_activity.promotion_items_title',
                'addfan_activity.promotion_items_url'
            )
    ...
    public function get_ad(Request $request)
    {
        ...
        $ad = DB::connection('rhea1-db0')->table('addfan_activity')
            ->select('ad_image_url', 'ad_url', 'ad_btn_url', 'ad_btn_text', 'ad_btn_color', 'ad_comment')
            ->where('id', $ad_id)
            ->first(); // prevent SQL injection
        ...
        $ad_comment = isset($ad) ? $ad->ad_comment : "_";
        $ad_result = array(
            "ad_image_url" => $ad_image_url, "ad_url" => $ad_url, "ad_btn_url" => $ad_btn_url,
            "ad_btn_text" => $ad_btn_text, "ad_btn_color" => $ad_btn_color, "ad_comment" => $ad_comment
        );
    ...
    // convert d to name and url form item_list and item_list_backup
